...
<?php
    ...
    class AllCategory
    {
        ...
        public $websiteName = "Clothes Store";
         ...
         */
         ...
         public function loadAllProduct($brandFilter = '', $categoryFilter = '', $genderFilter = '', $searchTerm = '')
        {
            $sql = "SELECT * FROM ecom_product";

            // Add JOIN clause to connect ecom_product, ecom_brand, and ecom_category tables
            $sql .= " LEFT JOIN ecom_brand ON ecom_product.brandid = ecom_brand.id";
            $sql .= " LEFT JOIN ecom_category ON ecom_product.catid = ecom_category.id";

            // Add WHERE clause based on brand and category filters
            $whereClause = "";

            if ($brandFilter) {
                $whereClause .= " ecom_brand.brand = '$brandFilter'";
            }

            if ($categoryFilter) {
                // Use AND if a brand filter is also present
                $and = $brandFilter ? " AND " : "";
                $whereClause .= "$and ecom_category.catname = '$categoryFilter'";
            }
            ...
            if($genderFilter){
                ...
                if($genderFilter=="Male" || $genderFilter=="Female"){
                    $whereClause .= "$and (gender = '$genderFilter' OR gender ='$all')";
                }else{
                    $whereClause .= "$and gender = '$genderFilter'";
                }
            }

            if ($searchTerm) {
                $and = $whereClause ? " AND " : "";
                $whereClause .= "$and (ecom_product.proname LIKE '%$searchTerm%' OR ecom_brand.brand LIKE '%$searchTerm%' OR ecom_category.catname LIKE '%$searchTerm%')";
            }
            ...
            $sql .= " ORDER BY ecom_product.proid DESC";
